    ...
    public class ProxyGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            if (context.SyntaxReceiver is ProxyDataReceiver receiver)
            {
                ...
                {
                    context.AddSource("[KnetworkingGenerated]" + type.TypeSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat).Replace("global::", ""),
        ...
        {
            ITypeSymbol proxyAttribute = context.Compilation.GetTypeByMetadataName("DouglasDwyer.Knetworking.MakeSharedAttribute");
            ...
            if (attribute != null)
            {
                ...
                context.AddSource("[KnetworkingGenerated]" + typeToShare.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat).Replace("global::", "").Replace("<","Of").Replace(">",""),
                    SourceText.From(GenerateRemoteTypeForInterface(typeToShare, "Remote" + typeToShare.Name.Replace("<", "Of").Replace(">", ""), "DouglasDwyer.Knetworking.Proxy." + (typeToShare.ContainingNamespace is null ? "" : typeToShare.ContainingNamespace.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat).Replace("global::",""))), Encoding.Default));
            }
        }

        private List<ProxyType> GetProxyTypes(GeneratorExecutionContext context, List<ClassDeclarationSyntax> candidateInterfaces)
        {
            ITypeSymbol proxyAttribute = context.Compilation.GetTypeByMetadataName("DouglasDwyer.Knetworking.ProxyForAttribute");
        ...
        {
            INamedTypeSymbol attributeSymbol = context.Compilation.GetTypeByMetadataName("DouglasDwyer.Knetworking.SharedAttribute");
            ...
            foreach (ClassDeclarationSyntax classDeclaration in candidateClasses)
            {
                ...
                if (classSymbol.GetAttributes().Where(x => x.AttributeClass.Equals(attributeSymbol)).Count() > 0)
                {
                    ...
                    toReturn.Add(new SharedType(classSymbol, "I" + classSymbol.Name, namespaceName, "Remote" + classSymbol.Name, "DouglasDwyer.Knetworking.Proxy" + (namespaceName == "" ? "" : "." + namespaceName), syntax));
                }
            }
            return toReturn;
        }
        ...
        const string proxyNamespaceTemplate = @"
                [assembly: global::DouglasDwyer.Knetworking.ShareAs(typeof({FULL_CLASS_NAME}), typeof({FULL_INTERFACE_NAME}))]
                namespace {NAMESPACE} {
        ...
        const string remoteNamespaceTemplate = @"
                /*namespace {REMOTE_NAMESPACE} {
                    [global::DouglasDwyer.Knetworking.ProxyFor(typeof({FULL_INTERFACE_NAME}))]
                    public class {REMOTE_CLASS_NAME} : global::DouglasDwyer.Knetworking.RemoteSharedObject, {FULL_INTERFACE_NAME} {
                        public {REMOTE_CLASS_NAME}(global::DouglasDwyer.Knetworking.SharedObjectPath location, global::DouglasDwyer.Knetworking.KnetworkingClient host) : base(location, host) {}                        
        ...
        const string proxyGlobalTemplate = @"
                [assembly: global::DouglasDwyer.Knetworking.ShareAs(typeof({CLASS_NAME}), typeof({FULL_INTERFACE_NAME}))]
                public partial class {CLASS_NAME} : {INTERFACE_NAME} {
