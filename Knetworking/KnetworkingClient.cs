...
[assembly: ShareAs(typeof(KnetworkingClient<>), typeof(IKnetworkingClient))]
[assembly: ShareAs(typeof(KnetworkingClient), typeof(IKnetworkingClient))]
namespace DouglasDwyer.Knetworking
{
    ...
    /// </summary>
    [Shared]
    public class KnetworkingClient : IKnetworkingClient
    {
        /// <summary>
        /// The remote server object, or local server object if this is a server-owned client.
        /// </summary>
        public IKnetworkingServer Server
        {
            get
            {
                if(RemoteServer is null)
                {
                    SendKnetworkingMessage(new GetRemoteServerObjectMessage());
        ...
        public KnetworkingSerializer Serializer { get; }

        /// <summary>
        /// The remote server reference.
        /// </summary>
        private IKnetworkingServer RemoteServer;
        ...
        /// <summary>
        /// The unique network ID of this client, used to identify this client from others connected to a <see cref="KnetworkingServer"/>. This ID is always 0 for server-owned objects.
        /// </summary>
        ...
        /// <summary>
        /// Creates a new <see cref="KnetworkingClient"/>.
        /// </summary>
        public KnetworkingClient() {
            LoadMethodCallbacks();
            Serializer = new KnetworkingClientSerializer(this);
            RemoteTaskScheduler = TaskScheduler.FromCurrentSynchronizationContext();
            Local = true;
        }

        /// <summary>
        /// Creates a new server-owned <see cref="KnetworkingClient"/>.
        /// </summary>
        ...
        internal KnetworkingClient(TcpClient client, KnetworkingServer server, ushort networkID, TaskScheduler scheduler)
        {
            LoadMethodCallbacks();
            Serializer = new KnetworkingClientSerializer(this);
        ...
        /// <param name="server">The server that owns this client.</param>
        /// <param name="networkID">The network ID of this client.</param>
        internal KnetworkingClient(TcpClient client, KnetworkingServer server, ushort networkID, KnetworkingSerializer serializer)
        {
            LoadMethodCallbacks();
        ...
        /// <summary>
        /// Attempts to connect to a KnetworkingServer using the specified IP address and port number.
        /// </summary>
        ...
        public void Connect(string ip, int port)
        {
            ...
            {
                throw new InvalidOperationException("The KnetworkingClient is already in use.");
            }
        }
        ...
        public Task<T> CallRemoteMethodAsync<T>(SharedObjectPath obj, object[] arguments, ushort methodID)
        {
            ...
            SendKnetworkingMessage(new CallRemoteMethodMessage(obj, methodID, arguments, operation.OperationID));
        ...
        {
            AsynchronousNetworkOperation<T> operation = CreateNewAsynchronousNetworkOperation<T>();
            SendKnetworkingMessage(new GetRemotePropertyMessage(obj, propertyName, operation.OperationID));
            return operation.Operation;
        }
        ...
        public async Task SetRemotePropertyAsync<T>(SharedObjectPath obj, T toSet, [CallerMemberName] string propertyName = null)
        {
            ...
            SendKnetworkingMessage(new SetRemotePropertyMessage(obj, propertyName, toSet, operation.OperationID));
        ...
        public void SendKnetworkingMessage(KnetworkingMessage message)
        {
        ...
        internal bool SerializeSharedObject(KnetworkingSerializer serializer, BinaryWriter writer, object obj)
        {
        ...
        internal object DeserializeSharedObject(KnetworkingSerializer serializer, BinaryReader reader)
        {
        ...
        {
            SendKnetworkingMessage(new ReturnRemoteServerObjectMessage(Server));
        }
        ...
        protected void ReturnRemoteServerObjectCallback(ReturnRemoteServerObjectMessage message)
        {
            if(RemoteServer is null)
            {
                RemoteServer = (IKnetworkingServer)message.Server;
            }
        }
        ...
        protected async Task CallRemoteMethodCallbackAsync(CallRemoteMethodMessage message)
        {
            ...
            else
            {
                if (message.InvocationTarget.OwnerID == NetworkID)
                {
                    try
                    {
                        SendKnetworkingMessage(new ReturnRemoteMethodMessage(message.OperationID, await new RemoteMethodInvoker(ShareAsAttribute.ProxyData[ShareAsAttribute.ProxyIndex[ShareAsAttribute.SharedTypes[toInvoke.GetType()]]].Methods[message.MethodID]).Invoke(toInvoke, message.Parameters), null));
                    }
                    catch (Exception e)
                    {
                        e = e.InnerException;
                        SendKnetworkingMessage(new ReturnRemoteMethodMessage(message.OperationID, null, new RemoteException(e.GetType().FullName + ": " + e.Message, e.StackTrace, e.Source)));
                    }
                }
                else
                {
        ...
        protected async Task GetRemotePropertyCallbackAsync(GetRemotePropertyMessage message)
        {
            ...
            else
            {
                if (message.InvocationTarget.OwnerID == NetworkID)
                {
                    ...
                    {
                        SendKnetworkingMessage(new ReturnRemotePropertyMessage(message.OperationID, null, new RemoteException(e.Message, e.StackTrace, e.Source)));
                        return;
                    }
                    SendKnetworkingMessage(new ReturnRemotePropertyMessage(message.OperationID, returnValue, null));
                }
                else
                {
                    throw new Exception("Client-client invocation is not supported at this time.");
            ...
            else
            {
                if (message.InvocationTarget.OwnerID == NetworkID)
                {
                    try
                    {
                        ...
                        SendKnetworkingMessage(new ReturnRemotePropertyMessage(message.OperationID, null, null));
                    }
                    catch (Exception e)
                    {
                        SendKnetworkingMessage(new ReturnRemotePropertyMessage(message.OperationID, null, new RemoteException(e.Message, e.StackTrace, e.Source)));
        ...
        protected async Task GetRemoteIndexerCallbackAsync(GetRemoteIndexerMessage message)
        {
            ...
            else
            {
                if (message.InvocationTarget.OwnerID == NetworkID)
                {
                    ...
                    {
                        SendKnetworkingMessage(new ReturnRemoteMethodMessage(message.OperationID, null, new RemoteException(e.Message, e.StackTrace, e.Source)));
                        return;
                    }
                    SendKnetworkingMessage(new ReturnRemoteMethodMessage(message.OperationID, returnValue, null));
                }
                else
                {
                    throw new Exception("Client-client invocation is not supported at this time.");
        ...
        private void RunCommunications()
        {
            try
            {
                ...
                while(true)
                {
                    ...
                    object o = Serializer.Deserialize<KnetworkingMessage>(networkReader.ReadBytes(messageLength));
    ...
    public class KnetworkingClient<T> : KnetworkingClient where T : IKnetworkingServer
    {
